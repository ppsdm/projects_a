...
class ProfileController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        $metas['education']['s1']['institution'] = '';
        $metas['education']['s1']['year'] = '';
        $metas['education']['s2']['institution'] = '';
        $metas['education']['s2']['year'] = '';
        $metas['education']['s3']['institution'] = '';
        $metas['education']['s3']['year'] = '';   
        
        $is_assesor = ProfileMeta::find()
        ->andWhere(['type'=>'global'])
        ->andWhere(['key'=>'role'])
        ->andWhere(['value'=>'assessor'])
        ->One();
        ...
        foreach ($profile_metas as $key => $value) {
            ...
            if($value->type == 'contact') {
               ...
               $metas['education'][$value->key]['institution'] = $value->value;
               $metas['education'][$value->key]['year']= $value->attribute_1;
    ...
    public function actionReviewassessor($id)
    {
        ...
        $result = ArrayHelper::map($assessors_model, 'profile_id', function($data){
            $profile = Profile::findOne($data['profile_id']);

            return $profile->first_name;
        });
        
            $review = new ProfileReview();
         
         if ($review->load(Yii::$app->request->post())) {
            $review->reviewer_id = $profi->id;
            $reviews =  ProfileReview::find()->andWhere(['assessee_id' => $review->assessee_id])->All(); 
        /*    if ($review->save()) {
        Yii::$app->session->addFlash('success', $profi->id . ' give review for ' . $review->reviewee_id);
            } else {
                        Yii::$app->session->addFlash('warning', 'failed');
            }
*/
         }


            return $this->render('assessor_review', [
                'assessors' => $result,
                'review' => $review,
            ]);
